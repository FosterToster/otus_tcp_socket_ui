use shtp::{DeviceType, Result as SHTPResult, SHTPClient};
use std::str::FromStr;

use iced::widget::{button, column, container, text, text_input};

use super::super::{ConnectMessage, Message};
...
impl ConnectView {
    fn try_connect(&self, host: String, port: u16) -> SHTPResult<SHTPClient> {
        ...
        client
            .send_command("test".to_string(), vec![])
            .map(|_| client)
    }
    ...
    fn parse_value(&self) -> Option<(String, u16)> {
        ...
        match u16::from_str(port) {
            Ok(val) => Some((
                host.iter()
                    .enumerate()
                    .fold(String::new(), |mut prev, (idx, cur)| {
                        if idx > 2 {
    ...
    pub fn update(&mut self, message: ConnectMessage) -> Option<SHTPClient> {
        match message {
            ...
            ConnectMessage::TryConnect => match self.parse_value() {
                Some((host, port)) => match self.try_connect(host, port) {
                    Ok(client) => {
                        self.error = None;
                        Some(client)
                    }
    ...
    pub fn view(&self) -> iced::Element<Message> {
        container(
            column(vec![
                match &self.error {
                    Some(err) => text(format!("Ошибка: {}", err)),
                    None => text("Введите адрес умной розетки"),
                }
                .into(),
                text_input("ip.ip.ip.ip:port", &self.value, |val| {
                    Message::Connect(ConnectMessage::AddrMutated(val))
                })
                .padding(3)
                .into(),
                button("Подключить")
                    .on_press(Message::Connect(ConnectMessage::TryConnect))
                    .into(),
            ])
            .spacing(5),
        )
